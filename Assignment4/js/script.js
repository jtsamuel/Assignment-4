...
function drawAxes(axisData) {
    ...
    const wedges = axesGroup.selectAll(".metric-wedge")
        .data(metrics, d => d);
    ...
    // 1. Radial axis lines
    // -------------------------
    const axisLines = axesGroup.selectAll(".metric-axis")
        .data(metrics, d => d);
    ...
    const labels = axesGroup.selectAll(".metric-label")
        .data(metrics, d => d);

    labels.join(
        enter => {
            const lbl = enter.append("text")
    ...
    const grids = axesGroup.selectAll(".radial-grid")
        .data(gridValues);
...
function drawlinesandBubbles() {
    ...
    const distLines = axesGroup.selectAll(".metric-distribution")
    ...
    const circles = axesGroup.selectAll(".region-bubble")
    ...
    circles.join(
        enter => enter.append("circle")
            ...
            .attr("r", d => d.region === RegionVar ? bubbleRadius * 1.2 : bubbleRadius)
            .attr("stroke-width", 2)
    ...
    axesGroup.selectAll(".region-bubble")
...
function highlightMetric(metric) {
    axesGroup.selectAll(".metric-wedge")
